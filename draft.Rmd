...
In order to investigate the relationship between student performance scores (G1_G2_Average) and the variables of interest, hypothesis testing was conducted using logistic regression models and confidence intervals were evaluated. The objective of this study was to analyze the effect of the following variables on student performance:
...
Here, we perform hypothesis testing and interpret the p-values for each predictor variable (School, Medu, Higher, and Absences) to evaluate their significance in association with the student performance scores. The code is adapted to include confidence intervals for a more comprehensive understanding of the results.
...
### Results
#### Coefficient Significance $(α = 0.05)$ / Confidence Intervals $(95\%)$
- **Intercept**: $\beta_0\$ \( [8.97, 11.4] \)

   We reject $H_0$ since the $p-value$ is $3.41 \times 10^{-38}$. This indicates that the intercept is statistically significant, suggesting that the baseline levels of the predictors have a meaningful contribution to the average student performance.

  
   Besides that we are 95% confident that the true value of the intercept lies within $[8.97, 11.4]$. Since the range is entirely > 1, it also suggests that the baseline group is associated with higher levels of student performance compared to other predictor categories.

- **School**: $\beta_1\$ \( [-2.58, -1.15] \)

  The $p-value$ $6.40 \times 10^{-7}$ is less than $0.05$, so we reject $H_0$. This result indicates that the type of school is significantly associated with average student performance.

  The confidence interval for School lies entirely below 0, which shows the same result.

- **Medu**: $\beta_2\$ \( [0.0418, 0.632] \)

  The $p-value$ $2.55 \times 10^{-2}$ is less than $0.05$, so we reject $H_0$. There is enough evidence to conclude that mother's education level is significantly associated with average student performance.

  The confidence interval didn't includes 0, also indicates evidence to conclude a relationship between mother’s education level (Medu) and higher student performance.

- **Higher**: $\beta_3\$ \( [0.826, 2.93] \)

  The $p-value$ $5.42 \times 10^{-4}$ is less than $0.05$, so we reject $H_0$. This suggests that planning to pursue higher education is significantly associated with improved average student performance.

  We can see the range $[0.826, 2.93]$ lies entirely > 0, also suggests a significant positive relationship between planning to pursue higher education degree and better student performance.

- **Absences**: $\beta_4\$ \( [-0.152, -0.0152] \)

  The $p-value$ $1.68 \times 10^{-2}$ is less than $0.05$, so we reject $H_0$. This indicates that absences are significantly associated with lower average student performance.

  The confidence interval lies entirely below 0, also suggests a significant negative association between the number of absences and higher student performance.
...
From this model, we got an $R^2$ value of 0.294, meaning that our model 
explains 29.4% of the variance in log of (G1_G2_Average). Additionally, 
this $R^2$ value did improve from our regular model by 0.0169. However,
the $R^2$ value our model achieve is not that high indicating that 
the relationship between target and predictor might not have a 
log-linear relationship between them.
...
log(G1_G2_Average) with estimate -0.18. This implies that being in
school MS (Mousinho da Silveira) as compared to school GP (Gabriel
Pereira) is associated with a decrease in log(G1_G2_Average) by 0.18
when other variables are constant \* Medu has a positive correlation of
0.0242, implying that for a unit increase in Mother's educational level,
the log(G1_G2_Average) increases by 0.0242 units. \* higherYes has a
positive correlation of 0.172, implying that an increase in 0.172 units in
...
correlation of 0.00665, implying that every additional day of absences is
associated with a decrease in log(G1_G2_Average) by 0.00665 units.
...
results for all four predictors, the low $R^2$ value of 0.294 suggests

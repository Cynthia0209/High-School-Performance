...
```{r include = FALSE}
echo=FALSE #load all the libraries 
...
library(leaps) 
required_packages <- c("rmarkdown", "knitr", "evaluate")
...
```{r echo = FALSE, fig.cap = "Table 1. Preview of the High School Alcoholism and Academic Performance dataset.\n"}
# load the dataset
...
```{r echo = FALSE, fig.cap = "Figure 1. Distribution of average of G1 and G2.\n"}
...
-   **School**: \$\beta\_1\$ ( \[-2.09, -0.741\] )

    The $p-value$ $5.35 \times 10^{-5}$ is less than $0.05$, so we
    reject $H_0$. This result indicates that the type of school is
...
-   **Absences**: \$\beta\_4\$ ( \[-0.199, -0.0629\] )

    The $p-value$ $2.01 \times 10^{-4}$ is less than $0.05$, so we
    reject $H_0$. This indicates that absences are significantly
    associated with lower average student performance.
